...
bool Market::isValidForPurchase(string testSelections[], int& column, int& row, const int& buyState)
{
    ...
    if(buyState == 0 && column > 3)
    {
        if(weaponsForSale || potionsForSale || armorsForSale)
            cerr << endl << "Warning: There are no more available Item slots." << endl;
        else
            cerr << endl << "Warning: Sorry, there are no available Items for sale." << endl;
        return false;
    }
    ...
    if(buyState == 1 && column < 4)
    {
        if(lightingSpellsForSale || fireSpellsForSale || iceSpellsForSale)
            cerr << endl << "Warning: There are no more available Spell slots." << endl;
        else
            cerr << endl << "Warning: Sorry, there are no available Spells for sale." << endl;
        return false;
    }
...
Market::Market() : lightingSpellsForSale(0), fireSpellsForSale(0), iceSpellsForSale(0),
                   weaponsForSale(0), potionsForSale(0), armorsForSale(0), minSpellPrice(INT32_MAX),
                   minItemPrice(INT32_MAX), minPrice(INT32_MAX), mostItems(0), prevLevel(0)
{
...
int Market::isAbleToBuy(const Hero* hero, const bool& info = true)
{
    ...
    if(hero->GetMoney() == 0)
    {
        if(info)
        {
            if(lightingSpellsForSale || fireSpellsForSale || iceSpellsForSale || weaponsForSale || potionsForSale || armorsForSale)
            {
                ...
                cerr << endl;
                cerr << "-------------------------------" << endl << endl;
                cerr << "Warning: Hero " << hero->Living::GetName() << " has no money. The minimum required amount is: " << minPrice << endl;
                usleep(2500000);
            }
            else
                cerr << endl << "Error: Hero " << hero->Living::GetName() << " has no money. The minimum required amount is: " << minPrice << endl;
        }
        return -1;
    }
    ...
    if(hero->GetMoney() < minPrice)
    {
        if(info)
        {
            if(lightingSpellsForSale || fireSpellsForSale || iceSpellsForSale || weaponsForSale || potionsForSale || armorsForSale)
            {
                ...
                cerr << endl;
                cerr << "-------------------------------" << endl << endl;
                cerr << "Warning: Hero " << hero->Living::GetName() << " doesn't have enough money. The minimum required amount is: " << minPrice << endl;
                usleep(2500000);
            }
            else
                cerr << endl << "Error: Hero " << hero->Living::GetName() << " doesn't have enough money. The minimum required amount is: " << minPrice << endl;
        }
        return -1;
    }
        ...
        if(hero->GetAvailableSpellSlots() == 0 || hero->GetMoney() < minSpellPrice)
        {
            if(info)
            {
                if(lightingSpellsForSale || fireSpellsForSale || iceSpellsForSale || weaponsForSale || potionsForSale || armorsForSale)
                {
                    ...
                    cerr << endl;
                    cerr << "-------------------------------" << endl << endl;
                    cerr << "Warning: Hero " << hero->Living::GetName() << " doesn't have any other available Item and Spell slots." << endl;
                    usleep(2500000);
                }
                else
                    cerr << endl << "Error: Hero " << hero->Living::GetName() << " doesn't have enough money. The minimum required amount is: " << minPrice << endl;
    ...
    if(!found)
    {
        if(info)
        {
            cerr << endl << "Error: There are no available Items or Spells for Hero " << hero->Living::GetName() << " to buy." << endl;
            usleep(3000000);
        }
        return -1;
    }
...
void Market::Purchase(Hero& hero, const int& buyState)
{
    ...
    if(buyState == 0)
    {
        cout << "Choose a Spell by selecting its position in the table(e.g.: <column> <row>): ";
    ...
    {
        cout << "Choose an Item by selecting its position in the table(e.g.: <column> <row>): ";
    ...
    {
        cout << "Choose an Item or a Spell by selecting its position in the table(e.g.: <column> <row>): ";
    ...
    if(column == 0)
    {
        cout << endl << "Info: The purchase was canceled." << endl << endl;
        return;
    }
    ...
    {
        case 1:
            cout << endl << "Are you sure you want to buy \"" << iceSpellsArray[iceSpellsIndexes[row - 1]]->GetName() << "\" Ice Spell? [y/n]: ";
        ...
        case 2:
            cout << endl << "Are you sure you want to buy \"" << fireSpellsArray[fireSpellsIndexes[row - 1]]->GetName() << "\" Fire Spell? [y/n]: ";
        ...
        case 3:
            cout << endl << "Are you sure you want to buy \"" << lightingSpellsArray[lightingSpellsIndexes[row - 1]]->GetName() << "\" Lighting Spell? [y/n]: ";
        ...
        case 4:
            cout << endl << "Are you sure you want to buy \"" << weaponsArray[weaponsIndexes[row - 1]]->GetName() << "\" Weapon? [y/n]: ";
        ...
        case 5:
            cout << endl << "Are you sure you want to buy \"" << armorsArray[armorsIndexes[row - 1]]->GetName() << "\" Armor? [y/n]: ";
        ...
        case 6:
            cout << endl << "Are you sure you want to buy \"" << potionsArray[potionsIndexes[row - 1]]->GetName() << "\" Potion? [y/n]: ";
...
void Market::Buy(vector<Hero *>& heroes)
{
    ...
    else
    {
        ...
        if(i == heroes.size())
        {
            cerr << endl << "Error: No one of the heroes are able to buy any Items or Spells from the Market.";
            usleep(2500000);
            DisplayMarket();
            return;
        }
    ...
    while(true)
    {
        ...
        else
        {
            ...
            {
                cerr << endl << "-------------------------------" << endl << endl;
                cerr << "Warning: There are no other heroes able to buy any Items or Spells from the Market.";
                usleep(3000000);
                DisplayMarket();
                return;
            }
            ...
            while(selectedHero == NULL)
            {
                ...
                while(true)
                {
                    ...
                    {
                        cerr << endl << "Error: Please choose a number in the given range." << endl;
...
int Market::isAbleToSell(const Hero* hero, const bool& info = true)
{
    ...
    if(hero->GetAvailableItemSlots() == hero->GetMaxItemsSlots())
    {
        if(hero->GetAvailableSpellSlots() == hero->GetMaxSpellsSlots())
        {
            if(info)
            {
                cerr << endl << "Error: Hero " << hero->Living::GetName() << " doesn't have any Items or Spells for sale." << endl;
                usleep(2500000);
            }
            return -1;
        }
...
void Market::Sale(Hero& hero, const int& sellState)
{
    ...
    if(column == 0)
    {
        cout << endl << "Info: The sale operation was canceled." << endl << endl;
        return;
    }
    ...
    switch(column)
    {
        case 1:
            if(sellState == 1)
            {
                cout << endl << "Are you sure you want to sell \"" << hero.GetWeaponFromSlot(row - 1)->GetName() << "\" Weapon? [y/n]: ";
            ...
            {
                cout << endl << "Are you sure you want to sell \"" << hero.GetSpellFromSlot(row - 1, 2)->GetName() << "\" Ice Spell? [y/n]: ";
        ...
        case 2:
            if(sellState == 1)
            {
                cout << endl << "Are you sure you want to sell \"" << hero.GetArmorFromSlot(row - 1)->GetName() << "\" Armor? [y/n]: ";
            ...
            {
                cout << endl << "Are you sure you want to sell \"" << hero.GetSpellFromSlot(row - 1, 3)->GetName() << "\" Fire Spell? [y/n]: ";
        ...
        case 3:
            if(sellState == 1)
            {
                cout << endl << "Are you sure you want to sell \"" << hero.GetPotionFromSlot(row - 1)->GetName() << "\" Potion? [y/n]: ";
            ...
            {
                cout << endl << "Are you sure you want to sell \"" << hero.GetSpellFromSlot(row - 1, 1)->GetName() << "\" Lighting Spell? [y/n]: ";
...
void Market::Sell(vector<Hero *>& heroes)
{
    ...
    else
    {
        ...
        if(i == heroes.size())
        {
            cerr << endl << "Error: No one of the heroes have any Items or Spells for sale.";
            usleep(2500000);
            DisplayMarket();
            return;
        }
    ...
    while(true)
    {
        ...
        else
        {
            ...
            {
                cerr << endl << "-------------------------------" << endl << endl;
                cerr << "Warning: No one of the heroes have any Items or Spells left for sale.";
                usleep(2500000);
                DisplayMarket();
                return;
            }
...
void Market::DisplayMarket(const bool& showInfo, const Hero* hero, const int buyState)
{
    ...
    {
        if(showInfo)
            cout << "|                                                               \033[4mAvailable Actions\033[0m: 1. Purchase, 2. Disable Info, 3. Cancel                                                                |" << endl;
        else
            cout << "|                                                                   \033[4mAvailable Actions\033[0m: 1. Purchase, 2. Info, 3. Cancel                                                                    |" << endl;
    }
    else
    {
        if(showInfo)
            cout << "|                                                              \033[4mAvailable Actions\033[0m: 1. Buy, 2. Sell, 3. Disable Info, 4. Exit                                                               |" << endl;
        else
            cout << "|                                                                  \033[4mAvailable Actions\033[0m: 1. Buy, 2. Sell, 3. Info, 4. Exit                                                                   |" << endl;
    }
    ...
    for(unsigned int i = 0, ice_i = 0, fire_i = 0, light_i = 0, wpn_i = 0, arm_i = 0, ptn_i = 0;
            i < mostItems; i++, ice_i++, fire_i++, light_i++, wpn_i++, arm_i++, ptn_i++)
    {
        ...
        if(ice_i < iceSpellsArray.size())
        {
            ...
            cout.width(23);
            iceSpellsForSale++;
            cout << left << iceSpellsArray[ice_i]->GetName() << "";
        }
        else
        {
            if(!terminationState)
        ...
        if(fire_i < fireSpellsArray.size())
        {
            ...
            cout.width(23);
            fireSpellsForSale++;
            cout << left << fireSpellsArray[fire_i]->GetName() << "";
        }
        else
        {
        ...
        if(light_i < lightingSpellsArray.size())
        {
            ...
            cout << left << lightingSpellsArray[light_i]->GetName() << "";
        }
        else
        {
        ...
        if(wpn_i < weaponsArray.size())
        {
            ...
            cout.width(23);
            weaponsForSale++;
            cout << left << weaponsArray[wpn_i]->GetName() << "";
        }
        else
        {
        ...
        if(arm_i < armorsArray.size())
        {
            ...
            cout.width(23);
            armorsForSale++;
            cout << left << armorsArray[arm_i]->GetName() << "";
        }
        else
        {
        ...
        if(ptn_i < potionsArray.size())
        {
            ...
            cout.width(23);
            potionsForSale++;
            cout << left << potionsArray[ptn_i]->GetName() << "|" << endl;
        }
        else
        {
...
/*
           ...
           wpn_i >= weaponsArray.size() && arm_i >= armorsArray.size() && ptn_i >= potionsArray.size())
        {
            ...
            if(buyState != -1)
            {
                cout << endl << " Hero: " << hero->GetName() << endl;
    ...
    /* ------------------------------- Start of 1st Line ------------------------------- */
            ...
            if(ptn_i < potionsArray.size())
            {
                cout << "|(i)| ~> Price: ";
                cout.width(15);
                cout << left << potionsArray[ptn_i]->GetPrice() << "|" << endl;
            }
            else
                cout << "|   |\033[26C|" << endl;
    ...
    /* ------------------------------- Start of 2nd Line ------------------------------- */
            ...
            if(ptn_i < potionsArray.size())
            {
                cout << "|(i)| ~> Required Level: ";
                cout.width(6);
                cout << left << potionsArray[ptn_i]->GetMinReqLevel() << "|" << endl;
            }
            else
                cout << "|   |\033[26C|" << endl;
    ...
    if(hero != NULL)
    {
        ...
        cout << " Level: " << hero->GetLevel() << endl;
        cout << " Money: " << hero->GetMoney() << endl;
        cout << " Items: (" << hero->GetMaxItemsSlots() - hero->GetAvailableItemSlots() << "/" << hero->GetMaxItemsSlots() << ")" << endl;
        cout << "Spells: (" << hero->GetMaxSpellsSlots() - hero->GetAvailableSpellSlots() << "/" << hero->GetMaxSpellsSlots() << ")" << endl;
    }
}
